...
const cleanString = (string) => {
    return (string !== undefined)
        ? string.replace(/\0/g, '').trim()
        : string;
}

const cleanAperture = (apertureValue) => Math.round(10 * Math.sqrt(Math.pow(2, apertureValue))) / 10;
...
const parseExif = (exif) => {
    return {
        ...
        createDateTime: exif.exif.DateTimeOriginal || exif.exif.CreateDate || exif.image.ModifyDate,
        camera: {
