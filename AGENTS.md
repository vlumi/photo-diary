...
- **API shapes come from the generated schema.** `react-app/src/lib/api-schema.ts` is generated from the server's OpenAPI document and never edited; `lib/api-types.ts` names what the app uses from it (`ApiPhoto`, `ApiGallery`, `ResponseOf<path, method>`). A service returns the generated type without a cast, and a component that needs a response shape derives it rather than writing an interface beside the call: a hand-written copy keeps compiling after the server changes, and a derived one does not. Two leftovers: the public `PhotoModel` / `GalleryModel` still declare their own input types (they narrow nullable date parts after validating, which wants a proper "validated photo" type), and `services/meta.ts` keeps its key list for run-time use, checked against the schema in both directions. Use an intersection, not `Omit`, to refine a schema type: they carry index signatures, and `Omit` over one collapses the known keys.
