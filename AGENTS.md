...
- **The top layer escapes the grayscale theme.** The monochrome effect is `MonochromeOverlay` — a `mix-blend-mode: saturation` overlay inside `#root`, not a `filter` (Gecko applies a root filter to the top layer too; a filter on any non-root element re-anchors every `position: fixed` descendant). Anything in the top layer — a `showModal()` `<dialog>`, a `popover`, a fullscreened element — paints *above* the overlay and shows in colour. The theme picker relies on this on purpose. Any other modal converted to `<dialog>` that shows content (photos, flags, map, thumbnails) needs an inset saturation overlay as its last child, or it leaks colour on the grayscale theme.
