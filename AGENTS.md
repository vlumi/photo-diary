...
- **Stats logic** is in `react-app/src/lib/stats.tsx` (the entry: types re-exported, `collectTopics`) and `react-app/src/lib/stats/`: `shared.tsx` for types and small helpers, `context.tsx` for the inputs of one `collectTopics` call plus the formatting and chart-shaping helpers bound to them, and one module per topic of the view (`general`, `time`, `gear`, `settings`, `image`, `light`), each a `build…Topic(context)`. `collectTopics` builds the per-category data; `StatsCategory.valueSortable` / `valueSortByLabel` mark which categories show the "By value / Top" toggle in their expanded modal. `Stats/EvolutionChart.tsx` is the stacked-area trend chart with the month / year granularity toggle (`useEvolutionGranularityStore`).
...
- **Admin surface** lives under `Manage/` (`/m/*` routes), gated by `user.is_admin`. Lazy-loaded out of the main bundle. Item routes open as centered modals over their list page — `ItemModal.tsx` is the shared shell (Frame + dirty-state confirm + Esc walk-up + close handling), each item form mounts inside it. Notable: `GalleryEdit.tsx` / `UserEdit.tsx` / `GroupEdit.tsx` own the form state, `GalleryItemShell.tsx` carries the Properties / Access tab nav inside the gallery item modal, `GallerySourcesSection.tsx` edits hybrid sources, `VirtualGalleryFilterSection.tsx` owns the saved-filter editor on a virtual gallery's own edit page, `SavedFiltersSection.tsx` is the parent gallery's directory of child virtual galleries, `Section.tsx` exports the shared `Section` / `SectionTitle` / `SectionHint` / `ModalHeader` primitives, `PhotoDrawer.tsx` opens as a routed modal via `<Outlet />`; it owns the form state and the editable sections, with `PhotoDrawer/` holding its styled components (`styles.ts`), the pure form logic (`form.ts`: the form's shape, parsing, and the patch an edit produces, unit-tested), small render helpers (`parts.tsx`), and the two sections that need little of that state as components (`OverviewSection`, `ReadonlySection`). Body scroll while any modal is open is locked via `stores/modal-stack.ts` (ItemModal pushes on mount, pops on unmount; `App.tsx`'s `BodyScrollLock` toggles `document.body.style.overflow`).

- **API shapes come from the generated schema.** `react-app/src/lib/api-schema.ts` is generated from the server's OpenAPI document and never edited; `lib/api-types.ts` names what the app uses from it (`ApiPhoto`, `ApiGallery`, `ResponseOf<path, method>`). A service returns the generated type without a cast, and a component that needs a response shape derives it rather than writing an interface beside the call: a hand-written copy keeps compiling after the server changes, and a derived one does not. The public models refine the schema's shapes rather than restating them: `PhotoModel`'s `PhotoData` is `ApiPhoto` with the date parts made certain, because its validator turns away a photo without a capture date, and sizes (nullable columns) are read through `sizeOf`, which says what a missing one means. `services/meta.ts` keeps its key list for run-time use, checked against the schema in both directions. To refine a schema type use an intersection, or strip its index signature first (see `KnownGallery` in `GalleryModel.ts`): the generated types carry one because the server may add fields, and `Omit` straight over it collapses the known keys.
...
- Pluggable DB via `DB_DRIVER` env (currently only `sqlite3`; tests run against `:memory:`). Driver implementation in `server/db/sqlite3/`; the public surface is `server/db/index.ts`. The driver is one module per entity (`meta`, `users`, `access`, `virtual-galleries`, `galleries`, `gallery-photos`, `photos`, `operations`) around `connection.ts`, which owns the one `better-sqlite3` handle, the schema helper and the test reset; `sqlite3/index.ts` only assembles the export. Modules import each other's functions freely (the cycles are harmless because nothing but `connection.ts` runs code at load), but a new top-level statement that *calls* into another module would not be.
...
- **OpenAPI.** `server/openapi.json` is generated from the route schemas by `@fastify/swagger` and committed; `react-app/src/lib/api-schema.ts` is generated from it in turn. After changing a route's schema run `npm run docs:dump` in `server/` and `npm run api:codegen` in `react-app/` — `tests/api/openapi.test.ts` fails on a stale copy. A route's `security` is documentation only (the token filter and the authorizer enforce access, and read neither): use the constants in `server/lib/api-docs.ts` — `SESSION` when a guest gets 403, `GUEST_OR_SESSION` when a guest may read. The shared 401 / 403 responses are added to the document by `documentAuthErrors`, not declared per route. The document is OpenAPI 3.1 (TypeBox writes JSON Schema 2020-12), and it is written for generated clients as well as the SPA — the iOS companion generates its client with Apple's swift-openapi-generator. So every operation has a deliberate `operationId` from `server/lib/operation-ids.ts` (it becomes a client method name: renaming one breaks client code), and nullable scalars go out as 3.1 type arrays (`type: ["number", "null"]`) via `nullableAsTypeArrays`, because generators drop the null half of `anyOf: [X, {type: "null"}]`. `tests/api/openapi.test.ts` refuses a route without a name and a leftover null union. Every route declares the success it sends: `response: CREATED` or `NO_CONTENT` from the same file for a bodiless 201 / 204, a schema otherwise. A route without one is documented as a placeholder 200 and, worse, sends whatever its handler returns unfiltered; `tests/api/openapi.test.ts` refuses it.
