...
- The OpenAPI document declares 3.1, which its content already was, names every operation, and writes nullable values the 3.1 way, so that client generators accept it: Apple's Swift generator rejected it outright before, and dropped nullability once the version was fixed.
...
- **Security:** `GET /api/v1/users/{userId}` (admin only) returned the user's raw database row, password hash and token-signing secret included; it now answers `id`, `name` and `isAdmin` like the user list, and a test reads every GET route as an admin and refuses any field that looks like a credential.
- **Breaking:** `PUT /api/v1/users/self/password` answers 204 with its fresh cookies, like every other update, instead of 200 with an empty object.
...
- Security bump `fast-uri` (#736) — transitive via Fastify's URL parsing stack. GHSA-7p8r-x3mc-p8w7.
...
Follow-up to rc.4's session-hardening pass. rc.4's 90-day `pd_access` Max-Age (#695) was the right fix in isolation, but it exposed two adjacent behaviors in the token filter that stranded the SPA when the browser was holding a stale access cookie. Plus one small photo-modal render race.
...
- New `/m/instance` page edits instance identity (`name`, `description`, `cdn`, `image`) and SPA runtime defaults via a `meta` table that takes precedence over the legacy `.env` keys (kept as fallback until 1.0). Closes #513.
- Esc anywhere in `/m/*` walks one step up the breadcrumb; stacked modals still close themselves first via capture-phase listeners. Closes #612.
- Manage item routes open as modals layered over their list page with a view/edit toggle and an edit→view→close→breadcrumb Esc walk-up; the gallery item modal merges Properties/Access tabs into one shared Frame. Closes #606.
- Manage view/edit surfaces use a shared bordered Section primitive; PhotoDrawer opens as a modal with prev/next nav, a combined Photo overview panel, split Gear/Exposure sections, and editable shutter (accepts `0.008` or `1/125`) + ISO. Closes #605.
- PhotoDrawer's dirty-state confirm now covers every close path (Esc / backdrop / X) in both routed and inline modes. Closes #624.

### Frontend

- Photo `disp_width` / `disp_height` columns dropped; the modal reads aspect ratio from `orig_width` / `orig_height` instead. Part of #262, closes #614.
- Photo modal `<img>` emits a `srcset` from the `photo_rendition` ladder so the browser picks the best fit per viewport. Closes #615.
- Converter renditions ladder is operator-configurable via the new `renditions` meta key (editable from `/m/instance`); intake writes each profile and registers a `photo_rendition` row. Closes #616.
- Global Stats (`/stats`, admin) gains the Evolution chart in every trendable category's modal. Closes #602.
- Stats Evolution chart adds `weekday` and `hour` as trendable categories. Closes #629.
- Manage list pages preserve scroll position when an item modal opens / navigates between siblings / closes, and the body underneath locks while a modal is up. Closes #630.
- `/m/photos` filters move into a modal with an active-filter strip in the header; the 360px sidebar + mobile FAB collapse into one filter UX across viewport sizes. Closes #631.
- Finnish localization cleanup: `geocoded_state_code` falls through `ISO3166-2-lvl4 → lvl6 → lvl7 → lvl8` (Estonia, Luxembourg, Liechtenstein), the per-language address blob drops when the city fails the script rule, and the now-redundant hand-curated `cities/` overlay layer is removed. Closes #485, closes #362.
...
- New `POST /api/v1/stats/evolution` (admin, unscoped) returns per-bucket year-month series across every gallery.
- `photo.is_private` + `user_gallery.can_see_private` / `group_gallery.can_see_private` gate the photo on every read path; flipped via `PUT /api/v1/photos/:photoId` and the `bin/{user,group,photo}.ts` CLIs.
- `GET /api/v1/photos/:photoId` opens to gallery-editors on at least one of the photo's galleries; the cross-gallery list `GET /api/v1/photos` stays admin-only.

### Operator

- `bin/instance.ts` prompts on a TTY after a fresh bootstrap to create the first admin user; `--auto` / non-TTY runs print the equivalent instructions. Closes #557.
- New `bin/photo-rerender.ts` feeds the `photo_rendition` table from disk via `scan` / `generate <name>` / `prune` subcommands, all dry-run unless `--apply`. Closes #617.
- Rendition layout collapses onto `photos/display/<maxDim>/<id>.jpg`: the `photo_rendition.name` column drops and the `renditions` meta key becomes a bare integer list. **Upgrade step:** `cd <instance>/photos/display && mkdir 1500 && mv *.jpg 1500/` before restarting the server past this version.
- `bin/instance.ts --edit` walks every configurable `.env` key, prompts for a new value (default = keep); SECRET is read-only (rotating it invalidates sessions). Closes #537 (the standalone slice).
...
- Hybrid gallery sources get an admin-UI surface. Real / hybrid galleries grow a "Source galleries" section on the manage page: a picker filtered to real galleries (the model enforces "sources must be real" — UI pre-empts), an ordered list with up / down / × per row, and a hint that clearing the list reverts the gallery from hybrid back to real. View mode shows source links; edit mode shows the picker + reorder controls. Saving with non-empty sources stamps `type='hybrid'` server-side via the existing `applyVirtualSources`; an empty array reverts. Closes #568.
...
- Filter widget gains range filters for continuous exposure categories: focal length, focal length (35mm-eq), aperture, exposure time, ISO, EV, LV. Each card renders a single horizontal list of formatted chips (`f/2.8`, `1/200`, `ISO 800`) drawn from the universe values; tap-an-endpoint designates that side as the "anchor" (the fixed end) and any other tap moves the free end — same gesture covers extending outward and shrinking inward, no modifier needed on PC or touch. The anchor chip inverts colors so it's unmistakable. A bold status line beneath the chips spells the current selection, and the strip shows the range chunk in the same vocabulary (`Aperture: f/1.4 – f/4`). Closes #264.
- Filter widget city chips read cleanly: the qualifier is appended only when two cities share a display name (matching the Stats path), and when it is, it's the localized region or country name rather than the raw subdivision code (`Springfield, Illinois` not `Springfield, US-IL`). Closes #593.
- Saved-filter editor consolidated onto each virtual gallery's own edit page: the `<Builder>` from the live filter widget is now prop-driven and shared, so the operator picks chips + ranges + dates with the same tool whether they're narrowing the public viewer or shaping a saved view. The source gallery's manage page becomes a directory of child virtual galleries (links + a "Create virtual gallery" button); the actual filter editing lives at `/m/galleries/<virtual-id>`, with the parent linked at the top and numeric ranges supported (joining `filter` + `dateRange` in `SavedFilterDefinition`). Topic sections in the Builder collapse to keep the editor compact, with an active-filter count badge per topic; the date-range card now shows a primary-color border when active and a bolder × clear button. Closes #563.
...
- `numericRanges` joins `filter` and `dateRange` in every photo/stats/filter-values request body — half-open `{min?, max?}` per kebab-case category, AND across categories. Evaluated by a new `matchesNumericRanges` predicate in `photo-filter-eval`. Threaded through `queryFilteredPhotos`, `queryFilteredPhotoCounts`, `queryFilteredPhotoNeighbors`, `queryGalleryFilterValues`, `queryGlobalFilterValues`, `getGalleryStats`, `getGlobalStats`, and `getGalleryEvolution`; cache-bypass mirrors the dateRange path.
...
- Saved filters / sub-galleries (#285): a saved filter is a third kind of gallery (`gallery.type='saved_filter'`) anchored to one real source gallery + a stored `(filter, dateRange)` baseline (`gallery_saved_filter` side table, migration 024). Surfaces in `/api/v1/galleries` like any other gallery, browseable at `/g/<saved-filter-id>` with the baseline applied server-side; user pills layer on top via the existing per-view filter envelope. CRUD endpoints stay nested under the source at `/api/v1/galleries/<gallery>/filters[/<id>]`. ACL inherits from source (extend, never narrow). Admin: `bin/gallery.ts filter list / add / remove / update` and the gallery edit page's Saved filters section. Closes #285.
- Date-range filter for per-view + stats endpoints: optional `dateRange: { from?, to? }` sibling of `filter` on every body that takes a FilterShape (gallery-photos `/query` / `/counts` / `/neighbors`, photos `/query`, gallery + global `/stats`). Both bounds inclusive `YYYY-MM-DD`, either side optional for half-open. Stats cache bypasses on bounded dateRange. Closes #264.
- Per-language metadata for photo title / description / place and gallery title / description: `photo_localized` (migration 021) and new `gallery_localized` (migrations 021 + 022) hold the overlay rows; the canonical column is read as the active default language's text but isn't reshuffled when the default changes (operator rotates content manually if they want it). `gallery.default_language` is NOT NULL with an `'en'` default; `createGallery` seeds new galleries from `process.env.DEFAULT_LANGUAGE`, which also surfaces through `/api/v1/meta`. Closes #281.
...
- New `GET /api/v1/filter-values?lang=…` endpoint — global flavor of the gallery-scoped `/gallery-photos/<id>/filter-values`. Same response shape (`categoryValues` + `byCityLocalized`), admin-only, rejected on hostname-bound instances. Cached under `:global:fv` (and `:global:fv:<lang>` for non-English) alongside the global stats entry; `invalidateGlobal()` sweeps both on photo writes. Drives the GlobalStats filter sidebar without that page having to load the cross-gallery photo array. (followup to #532)
...
- Date-range filter pill in the public gallery + GlobalStats Filters panel (#264). Two native date inputs in an indented stack, half-open allowed (either side optional). Hooks into `useFiltersStore.dateRange` and flows alongside the existing `filter` field into every per-view / stats fetch body.
- Admin UI per-language inputs for photo title / description / place and gallery title / description, plus a "Primary language" dropdown on the gallery form that writes `gallery.default_language`; the default language's overlay row is hidden (canonical input carries that language and labels with the code). Only changed entries land in the PUT body. Closes #343.
...
- Global Stats Galleries section honors the Filters sidebar. The section now reads the per-gallery filtered counts off `/stats`'s new `byGallery` field instead of walking the cross-gallery photo array client-side. Same queryKey as the sibling `<Stats globalScope>` fetch, so TanStack dedupes — one network call for both sections. Empty `byGallery` (no galleries hit by the active filter) hides the section. (closes #446)
...
- Sidecar intake no longer creates a phantom photo when a `.JPG`-extension file is processed and the operator's JSON sidecar references the lowercase `.jpg` form. The converter now lowercases the extension on `originalFilename` at intake (so the DB stores `foo.jpg` regardless of whether the camera wrote `foo.JPG` or `foo.jpg`), and the `loadPhotosByOriginalFilename` lookup uses `COLLATE NOCASE` as a belt-and-suspenders safety net for any mixed-case rows already in the DB. Photos written before this fix may need a one-shot SQL cleanup — see the PR body for the query.

### Tooling

- `bin/instance.ts` runs the pm2 cycle directly — upgrade mode auto-cycles; new `--cycle` flag in doctor mode handles in-version code changes (e.g. `git pull` against a checkout the `code` symlink already points at). TTY-confirm + `pm2 jlist` sanity check + post-cycle `/api/v1/meta` probe; `--auto` for unattended, `--print-only` falls back to printed instructions. Doctor output reports installed pm2 version; README adds pm2 to prerequisites. Closes #546, #551.
...
- New `GET /api/v1/photos/audit-counts` endpoint (admin) returning per-predicate tallies (`orphan`, `duplicates`, `countryMismatch`, plus `missing.<field>` for every `MissingField` the filter chips support) over the in-scope catalog. Keys match the photos-page query-string params so the Manage dashboard's audit tiles deep-link to `/m/photos?<key>=…` with the matching chip pre-active. (part of #454)
...
- Home button always lands on the tile picker. The no-galleryId redirect ladder (single-visible-gallery → `DEFAULT_GALLERY`) moves off `/g` and onto `/` (the bare-instance landing). `/g` becomes pure picker. Direct visits to `/` keep the smart-redirect behavior; clicking Home from inside a gallery (on instances with a `DEFAULT_GALLERY` or a single visible gallery) no longer bounces straight back into the same gallery. Manage and Global Stats Home buttons retarget to `/g` for the same effect. (closes #468)
...
- Manage admin UI mobile / small-screen pass: Access / Galleries / Users / Groups tables wrap in an `overflow-x: auto` `TableScroll`, Photos page renders Prev / Next pagers both above and below the grid with 44px touch targets, Access table drops its Type column for a user/group icon prefix on the Subject id and centers the Editor + Map controls under their headers, the row-remove × swaps to a trash icon, `Th` / `Td` padding tightens under 640px, "Map visibility" → "Map" / "Inherit" → "Default" in en/fi/ja, and the Manage breadcrumb gains a gear icon to match the `GlobalStats` parity. (closes #412)
...
- Global Statistics page at `/s` — admin-only, aggregates every photo in the catalog (paginated fetch under the existing `/api/v1/photos` endpoint) through the same Stats / Filters UI the per-gallery `/s/<gallery>` view uses. The shared `uniqueValues` build moves out of `Gallery/index.tsx` into `lib/uniqueValues.ts` so the two pages can't drift. (part of #404)
...
- Admin Photos page (read-only) — `/m/photos` (cross-gallery) and `/m/g/<gallery>/photos` (gallery-scoped) render a paginated thumbnail grid with a faceted filter sidebar (gallery membership chips, date range, audit chips, free-text); filter state is URL-parameterized so a narrowed view is shareable. (part of #10)
...
- Drawer country field becomes a typeahead picker — reuses the lang-store's `i18n-iso-countries` instance, no new dependency or network call. Stores the alpha-2 code on the form; filters by localized name or code as you type; "Clear country" option above the list unsets. (closes #414)
- Drawer Location section gains an embedded Leaflet map — the marker is draggable and click-on-map repositions it; both write the new coordinates back into the lat/lon inputs. Typing into the inputs recenters the map. Bundled lazily so the leaflet payload doesn't load until the admin opens a photo. (closes #413)
...
- Photo modal drag past a gallery edge now stays bounded. The carousel `dragConstraints` were expressed in the wrong reference frame (relative to translate-origin 0 rather than the track's resting `motion.x = -window.innerWidth`), so dragging from the last (or first) photo immediately snapped the track toward 0 and exposed the neighboring slide. Corrected to `[-2 × innerWidth, 0]` with edge variants that lock at rest.
...
- `bin/photo.ts` lookup-or-create helper extracted to `server/lib/photo-intake.ts` so the converter's JSON-sidecar pipeline shares the same matching chain (exact id → originalFilename + timestamp → ambiguous fallback). No behavioral change for `photo.ts` callers.
- Top-level `audit-cities`, `cleanup-localized-cities`, `normalize-cities` consolidated under a single `cities <action>` parent: `cities audit`, `cities normalize`, `cities clean-localized`. Same behavior per action; one help-screen line instead of three.
...
- New `db.loadPhotosByOriginalFilename(name)` driver method (sqlite3 + dummy) so the lookup chain and `bin/photo.ts search` can find photos by their human-recognized camera filename.
...
- Stats Location card splits the geotagged count into two click-to-filter chips when the gallery has a mix — `N geotagged` and `M not geotagged` chips apply a `general / geotagged / yes|no` filter to the whole Stats view (same toggle behavior as the existing Country / Camera tables). Uniform cases collapse to a single line (`All N photos geotagged` / `No photos geotagged (of N)`); no filter affordance when there's nothing to filter on. Backed by a new `geotagged` case in `PhotoModel.matches`. (closes #336)
...
- Photo modal metadata moves to a toggleable corner panel so the photo fills the modal's full vertical space; date/time stays persistently visible in the toolbar center. (closes #306)
...
- Gallery Title bar carries a clickable breadcrumb path (`🏠 › Gallery › 2024 › March › #1234`); the Navigation bar below reorganizes into left/right control groups. (closes #275)
...
- Title bar gains keyboard shortcuts: `m` toggles the map modal, `g` jumps to Gallery, `s` jumps to Stats — the `g` path honors the remembered last-gallery URL. Shortcuts skip when an input / select / textarea is focused so the gallery dropdown's native letter-search keeps working. (closes #327)
