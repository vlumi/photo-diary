...
- Rename and relocate the operator scripts. The files in `server/bin/` lose the misleading `add-` prefix (they were upserts, never just creates) and gain a more accurate bare-noun naming: `add-photo.ts` → `photo.ts`, `add-gallery.ts` → `gallery.ts`, `add-user.ts` → `user.ts`, `init-instance.ts` → `instance.ts`. The repo root grows a `bin/` directory of bare-name symlinks (`bin/photo`, `bin/gallery`, `bin/user`, `bin/instance`) so the `server/` segment that wasn't carrying meaning drops out of the invocation path. The `instance` script populates a per-instance `<instance>/bin/{instance,photo,gallery,user}` directory of symlinks on init / doctor / upgrade, so the day-to-day operator commands become `./bin/photo …` from inside the instance dir instead of `./code/server/bin/add-photo.ts …`. Docs updated throughout.
