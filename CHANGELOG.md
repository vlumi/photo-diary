...
- Stats page language switch no longer rebuilds chart.js datasets. The language-independent build phase of `collectTopics` (sort, truncate, ranks, colors, dataset values) is now cached per raw-bucket reference + theme via a module-level WeakMap; only labels and table cells regenerate per language. The chart `data.datasets` array carries the same reference across calls, so chart.js does a fast labels-only update instead of a full rebuild. Closes #231.
