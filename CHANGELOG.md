...
- Stats expanded-table modal gains a sort toggle for value-sorted exposure categories (focal length, aperture, exposure time, ISO, EV, LV, resolution) — "By value" keeps the natural ascending/descending sort, "Top" re-orders chart and table by count desc so the most-used values surface at the top. Toggle renders only on categories where the choice is meaningful; chronological categories keep the calendar order.
