...
- No-gallery landing ladder gains two role-aware steps. After the existing single-visible-gallery short-circuit, a global admin on the primary (unbound) host now redirects to `/m` (admin home); a bound hostname still narrows them to that gallery via virtual-host scope (#386). Below the admin step, signed-in returning users get redirected to their last-viewed gallery — `lastGalleryId` persists to localStorage via the `last-gallery-path` zustand store; per-gallery deep paths stay in-memory by design so a hard reload returns them to the gallery root, soft navigation returns them to the exact URL they left. If the remembered gallery is no longer accessible (revoked grant, deleted gallery) the ladder falls through to `DEFAULT_GALLERY` and finally the tile picker. (closes #387)
