...
- Photo modal `Navigation` row pins `min-height: 50px` to absorb a one-frame sub-pixel height jitter (~0.3 px) at the navigate() commit and at modal mount. Without the pin, the carousel viewport grew briefly when the photo prop changed (or when the modal first mounted) and the newly-mounted Content measured the expanded size — visible as a brief stretch of the outgoing photo at the end of the slide animation, and a stretched-then-shrinks photo when opening the modal from Month view.
- Gallery `Navigation` shared component now forwards `className`, so `styled(SharedRoot)` overrides used by per-view Navigations (Photo, Month, Year) actually reach the inner `<Root>` element instead of being silently dropped on the floor. The Photo modal's `min-height` pin only works after this fix.
- Photo modal `Content` measures container dimensions in `useLayoutEffect` instead of `useEffect`, so the corrective rect read happens before the first paint. Previously the initial render painted with the full-window fallback dimensions and only shrunk on the next frame when the effect ran — visible as a brief full-modal stretch when opening the modal from Month view.
