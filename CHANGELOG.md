...
- Admin Photos page gains multi-select with bulk actions. Selection is always-on (no mode toggle): clicking a tile toggles its selection, shift-click extends a range from the last toggle anchor, and the result-summary row exposes a `Select page` / `Clear page` shortcut. Each tile carries a small pencil icon (top-right corner, same `BsPencilSquare` glyph the public photo view's manage-this-photo button uses) that opens the edit drawer; clicking the rest of the tile selects without opening. When the selection is non-empty, a bulk-action bar appears above the grid with Unlink from gallery, Delete, and (cross-gallery view only) Link to gallery. Each action opens a centered modal (confirm for Delete, gallery picker for Link / Unlink) and loops the existing per-row endpoint sequentially with inline progress + error. The Link button is hidden inside `/m/g/<id>/photos` — linking is conceptually a cross-gallery action and belongs on `/m/photos`. (part of #10)
