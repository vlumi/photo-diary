...
- Converter dedup at intake reuses the existing row's id rather than creating a duplicate. Two cases handled: a Lightroom re-export of the same shot (matching `originalFilename` + EXIF `DateTimeOriginal`) overwrites display/thumbnail/original/sidecar in place and refreshes the row's EXIF-derived columns; a JSON-first stub (operator's `bin/photo.ts <coords.json>` arrived before the SOOC) is merged onto when the SOOC finally lands. Opinion columns (title, country, place, author, description) are preserved in both cases — `photoMapToRow` only touches columns present in the input. EXIF-less re-imports still create new rows (mtime isn't strong enough to be confident).
- `bin/photo.ts` defaults `originalFilename = id` when creating a new row, so existing `{ id, taken: { location: { coordinates }}}` operator JSONs flow through the JSON-first stub path without needing an explicit `originalFilename` field.
