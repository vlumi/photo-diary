...
- Admin Photos page gains a Select mode — toolbar `Select` button flips the grid into multi-select; clicking tiles toggles selection (shift-click extends the range from the last toggle), the result-summary row exposes a `Select page` / `Clear page` shortcut, and a bulk-action bar surfaces Unlink from gallery, Delete, and (cross-gallery view only) Link to gallery. Each action opens a centered modal (confirm for Delete, gallery picker for Link / Unlink) and loops the existing per-row endpoint sequentially with inline progress + error. The Link button is hidden inside `/m/g/<id>/photos` — linking is conceptually a cross-gallery action and belongs on `/m/photos`. Default click behavior (open the edit drawer) is unchanged out of select mode. (part of #10)
