...
- Manage Photos grid gains long-press to enter selection on touch (~400ms, gated off mouse so desktop click still opens the drawer) and a mouse-only click-and-drag marquee rect from the grid background that toggles intersected tiles on release (shift-drag toggles against the pre-drag baseline instead of unconditionally adding). (closes #452)
