...
- Admin Photos page gains a Select mode — toolbar `Select` button flips the grid into multi-select; clicking tiles toggles selection (shift-click extends the range from the last toggle), the result-summary row exposes a `Select page` / `Clear page` shortcut, and a bulk-action bar surfaces Delete, Link to gallery, and Unlink from gallery actions. Each action opens a centered modal (confirm for Delete, gallery picker for Link / Unlink; the picker defaults to the current gallery on `/m/g/<id>/photos`) and loops the existing per-row endpoint sequentially with inline progress + error. Default click behavior (open the edit drawer) is unchanged out of select mode. (part of #10)
