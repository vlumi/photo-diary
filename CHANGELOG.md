...
- Manage Photos grid gains a touch / pen multi-select gesture: long-press a tile (~400ms, gated off mouse so the desktop click → drawer flow is unchanged) toggles its selection and immediately enters paint-sweep mode, where dragging the finger across other tiles applies the same outcome (select or deselect) to every tile it crosses; once selection is on, any finger drag from any tile starts the sweep without needing a second long-press. The page auto-scrolls while the finger sits within 80px of the top or bottom edge of the viewport so a many-tile selection doesn't need a lift-and-scroll cycle. (closes #452)
