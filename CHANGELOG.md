...
- No-gallery landing ladder remembers the per-user last-viewed gallery. After the existing single-visible-gallery short-circuit and before `DEFAULT_GALLERY`, signed-in returning users now get redirected to where they were instead of dumping them at the picker every time. `lastGalleryId` persists to localStorage via the `last-gallery-path` zustand store; per-gallery deep paths stay in-memory by design, so a hard reload returns them to the gallery root (which then auto-redirects to the latest month) and soft navigation returns them to the exact URL they left. If the remembered gallery is no longer accessible (revoked grant, deleted gallery) the ladder falls through to `DEFAULT_GALLERY` and finally the tile picker. Admins land on the picker the same as everyone else — the landing page's existing `Manage` shortcut card is one click away. (closes #387)
