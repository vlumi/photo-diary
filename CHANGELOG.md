...
- Photo modal geocoded line is now hand-assembled per UI language from the structured `geocoded_country_code` / `state` / `city` / `district` fields instead of Nominatim's raw `display_name`. Western locales (en, fi) render `district, city, state, country` comma-separated; Japanese renders `country state city district` space-separated (space rather than nothing, so all-katakana or mixed-script values don't run together). Postcode and irrelevant intermediate levels Nominatim included in some locales are gone. Tidying the operator location row to avoid the country appearing on both rows when the operator has no `place` is left to the MetadataPanel layout rework.
