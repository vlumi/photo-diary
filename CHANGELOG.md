...
- New opt-in beta surface for state-level data, gated by a per-browser toggle in the UserMenu ("Beta features"). When enabled, the photo metadata address line picks up the state between city and country, a new State filter category appears in General (with flag + localized name), Stats gets a State table between Country and City, and the Summary panel adds a States variety count + Top State tile. State names are resolved from the language-neutral `geocoded_state_code` (ISO 3166-2) through curated repo-local JSON files at `react-app/src/lib/translations/subdivisions/{en,fi,ja}.json` — hand-maintained, no external dependency, dynamically imported per language so the main bundle stays lean. Initial coverage: Japan's 47 prefectures, Finland's 19 regions, Germany's 16 Bundesländer, Malaysia's 16 states + federal territories, South Korea's 17 subdivisions, Australia's 8 states + territories, Canada's 13 provinces + territories, and all 50 US states + DC (en + ja; fi has Finnish-region names and falls back to en for the rest, since Finnish convention typically uses English forms for foreign subdivisions). Lookup falls back through `requested → en → ISO code`.
