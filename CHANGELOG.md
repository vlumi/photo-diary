...
- Manage dashboard grows a "Catalogue audit" tile strip above the navigation tiles, one card per non-zero filter-chip predicate (orphan, duplicates, country mismatch, missing-{taken,coords,place,country,author,title,description,state-code}). Clicking a card lands on `/m/photos` with the matching chip pre-active. Cards with count zero are hidden so the strip reads as "what's still to fix". Drawer save, delete, regeocode, and bulk actions invalidate the count query so the strip stays current as the operator works. (closes #454)
