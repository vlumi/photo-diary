...
- Admin Photos edit drawer — click a tile or visit `/m/photos/<id>` / `/m/g/<gallery>/photos/<id>` to open a side panel with the operator-editable fields (title, description, author, country, place, camera-{make,model}, lens-{make,model}, focal, aperture); EXIF / geocoded columns surface read-only; saves go through `PUT /api/v1/photos/<id>`; matching `missing=…` filter chips highlight the empty field and a hint explains the match. (part of #10)
