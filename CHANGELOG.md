...
- Photo `disp_width` / `disp_height` columns dropped; aspect ratio for the Photo modal layout reads from `orig_width` / `orig_height` instead (uniform downscale preserves it). Wire shape loses `dimensions.display`. Side-effect cleanup: the Photo modal's `<img>` styled-component had a bogus CSS rule (`width: ${props.width}` with no unit, silently dropped by the browser) that was "working" only because the same value was set on the HTML attr right next to it; the CSS rule is gone, the HTML attrs stay, so the browser's aspect-ratio reservation is the single source for the slot's pre-paint size. Part of #262 / closes #614.
- Photo modal `<img>` now emits a `srcset` built from the new `photo_rendition` ladder, letting the browser pick the best fit; ladder is keyed by `(photo_id, name, max_dim)` so any future per-photo render at any size joins the srcset automatically (current ladder is just `display @ 1500`). Closes #615.
- Converter renditions ladder is now operator-configurable via the new `renditions` meta key (JSON-encoded `[{name, maxDim}]`, editable from `/m/instance`); the converter reads it on every intake, writes each profile to `<name>/<id>.jpg` with `fit: inside`, and inserts a matching `photo_rendition` row. Unset / empty / malformed → fallback to the built-in `[{name: "display", maxDim: 1500}]`. Existing photos aren't backfilled by adding a row (use `bin/photo-rerender.ts`, #617). Closes #616.
