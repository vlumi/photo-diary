...
- GlobalStats no longer fetches the full cross-gallery photo array at mount. The filter pill universe comes from the new global `/filter-values` endpoint; the empty-instance signal reads off the response's `categoryValues.year`. The full-photos fetch (paginated `fetchAllPhotos`) survives but is gated on `mapPhotosWanted` state — flipped by a callback `Stats` invokes when the user opens the Location MapModal, mirroring the gallery-scoped lazy-fetch pattern. Brief empty-map flash on first open while the fetch completes; subsequent opens hit the TanStack cache. The legacy `buildUniqueValuesFromPhotos` photo-walking helper in `lib/uniqueValues.ts` is deleted (no callers). (followup to #532)
