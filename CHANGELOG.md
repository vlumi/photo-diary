...
- Photo modal metadata moves from a bottom Footer to a toggleable corner panel. The photo now fills the modal's full vertical space; details (title / location / coordinates / exposure / gear / epoch info / map / copyright) live behind an info icon at the photo's bottom-right corner. The panel defaults to visible on desktop and hidden on mobile so the photo gets every pixel on small screens. Date/time stays persistently visible in the toolbar centre — alongside the existing `#index / total` position indicator — so the most useful caption is always one glance away without opening the panel. (closes #306)
- Photo view restructures as a modal overlay on top of its parent Month view. Closing the modal returns to the Month exactly as the user left it (scroll position, filter UI state) with no remount. Direct-link entries to a photo URL still work — Month renders synchronously underneath, then the Photo modal opens on top. New close `[X]` affordance in the top-right corner; Escape and the existing back paths still close the modal. (part of #276 — architectural change; visual refinement and metadata-corner work to follow)
- Gallery Title bar now carries a breadcrumb path — `🏠 › Gallery › 2024 › March › #1234` — so the navigation hierarchy is always visible and every level is one click away. Each segment is a link to its level, except the current view (bold, non-interactive). The Navigation bar below loses its centered title (the path makes it redundant) and reorganises into left/right control groups. On Photo, the modal's close `[X]` joins the existing fullscreen toggle in a small "tools" sub-cluster on the right side of the Navigation bar — the previously floating absolute-positioned X is gone, the chrome reads as one bar. Escape keyboard shortcut still works at every level. (closes #275)
- Stats inline tables always sort by count desc — every category reads as a "top N" regardless of how many rows it has. Time and exposure categories that previously stayed in their natural order (Mon-Sun, ascending focal length, …) now share the same top-by-count default in the main view as the rest. The natural order is still available one click away.
- Stats expanded-table modal gains a sort toggle on every data category, defaulting to "Top" (count desc — matches the inline view the user just came from). "By value" is the alternate reading: chronological for time categories, ascending/descending numeric for exposure ranges, alphabetical for gear and people-or-place categories.
- Stats Summary category gains an expand modal with a richer overall-stats view organised into four sections: Period (first/last/span/totals), Peaks (busiest year/month-name/weekday/hour), Variety (distinct-value counts per category), Most common (top entry per author/country/camera/lens/focal-length/aperture/exposure-time/ISO). Peaks and Most common use tie-aware rendering — a single leader, a 2–3-way tie listing all winners, and 4+ near-tied or near-flat distributions (typical for evenly-spread categories like weekday on a 365-style daily diary) as "Evenly distributed" rather than picking a fake leader. All four sections share a single responsive tile grid; counts include their share of the total as muted meta.
