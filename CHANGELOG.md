...
- Calendar navigation gets a visible up-one-level affordance at every level. The centered title in each `<Navigation>` bar (Photo / Month / Year) now reads as a button: a small return-arrow icon prepended, a hover border that signals interactivity, and an aria-label / tooltip ("Back to month" / "Back to year" / "Back to galleries"). The Escape shortcut and the existing title-click destination stay unchanged — this is purely a discoverability fix. (closes #275)
