...
- Photo modal swipe becomes touch-tracking via framer-motion — the photo follows the finger horizontally, rubber-bands at the gallery's first/last photo, commits to next/prev on release past ~30 % of the viewport or with sufficient flick velocity. As a prerequisite, all in-app navigation between sibling photos / months / years (and the Title-bar context switcher) moves from `setRedirect → <Navigate>` (which unmounted the modal between transitions) to `useNavigate()` (URL changes in place, the modal stays mounted) — keyboard arrow nav and prev/next button clicks no longer flash the underlying view. (closes #175)
