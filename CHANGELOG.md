...
- Admin photos: bulk-action bar grows a "Set field…" button alongside Link / Unlink / Delete; modal picker covers the operator-writable fields (title / description / author / country / place / camera-make / -model / lens-make / -model / focal / focal35mm / aperture), string fields clear on empty input, numeric fields require a value, then loops `PUT /api/v1/photos/<id>` over the selection. (closes #451)
