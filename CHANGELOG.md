...
- ACL user groups — adds a `group` identity, `user_group` membership, and `group_gallery` grants. Group rows compose into the access cascade as one more positive-grant source in the MAX over `(user, gallery)` / `(group_X, gallery)` / `(:guest, gallery)`. `hide_map` resolves user-row → group-row (privacy-first MAX within the layer) → `:guest`-row. Migration 013 adds the three tables with FK cascade so deleting a user or group cleans up its memberships and grants automatically. New `/api/v1/groups` (CRUD + members) + `/api/v1/group-gallery` (mirror of `/user-gallery`). CLI surface redistributed along the three entities: `bin/access.ts` retires; user-side ACL (`grant` / `revoke` / `hide-map` / `grants` / `access`) lives in `bin/user.ts`, group-side in `bin/group.ts`, and `bin/gallery.ts access [<gallery>]` shows the per-gallery view of who has access. `bin/user.ts access [<user>]` is the effective-access view that resolves direct + group + `:guest` sources for debugging "why can/can't X see Y?". `bin/instance.ts` sweeps the stale `bin/access.ts` symlink on the next doctor run.
