...
- Cross-host SSO endpoints for the upcoming virtual-host switcher (#664, server side): `POST /api/v1/tokens/cross-host` mints a short-lived HMAC-signed token; `GET /api/v1/tokens/sso` consumes it on the target host, sets normal auth cookies, and 302s to the in-app redirect path. New `SSO_SECRET` env var enables the feature (same value across all sibling instances); unset → both endpoints 503. New `knownHosts` meta key carries the JSON list of `{hostname, label, isMain}` entries the SPA's switcher will render. Client side lands in #664 follow-up.
...
- Finnish localisation cleanup: `geocoded_state_code` falls through `ISO3166-2-lvl4 → lvl6 → lvl7 → lvl8` (Estonia, Luxembourg, Liechtenstein), the per-language address blob drops when the city fails the script rule, and the now-redundant hand-curated `cities/` overlay layer is removed. Closes #485, closes #362.
