...
- Virtual-host admin scope: requests reaching the server with a `Host` header matching a gallery's `hostname` pattern are now treated as administratively scoped to that gallery (or to the set of galleries when multiple match). Cross-gallery admin operations (user CRUD, gallery CRUD, instance meta mutations) 404 from a scoped host. `PUT /galleries/:id`, `PUT/DELETE /photos/:id`, and `user-gallery` mutations are accepted only when the target gallery is in scope. `GET /user-gallery` returns rows narrowed to the scope. Public reads (gallery list, photo metadata) are unaffected. The SPA mirrors the scope client-side: the breadcrumb gallery dropdown filters to the bound galleries, and an off-scope URL redirects in.
