...
- Stats category tables cap at 12 rows inline; a trailing "+ N more…" row opens the full distribution in a floating modal. Keeps the main Stats page scannable without hiding the long tail — every entry is still one click away, and the modal still supports the click-to-filter behavior the inline table has.
