...
- [**1.0**](https://github.com/vlumi/photo-diary/milestone/4) — Soak the accumulated security + polish work in prod, then stamp. `1.0.0-rc.5` cut 2026-07-08 (token-filter follow-up: rc.4's longer `pd_access` Max-Age exposed a stale-cookie stranding path; skip verification on non-API URLs + recovery/public endpoints so the SPA's refresh flow triggers cleanly). Feature freeze until 1.0; rc.6+ ships only if further verification surfaces regressions.
