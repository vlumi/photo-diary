...
The three pieces communicate via the shared filesystem and SQLite DB rather than over a network — the converter writes one thumbnail and one or more display renditions per intake (the ladder is operator-configurable via the `renditions` meta key; the default is a single `display @ 1500`) and inserts the photo row into the DB on intake, processing operator JSON sidecars through the same pipeline; the server reads the DB to serve the API; `bin/photo.ts update <id>` is the operator hook for after-the-fact enrichment. Per-instance state (database, photo files, `.env`) lives in a single instance directory outside the repo; see [Setup](#setup) below.
...
5. **Set the instance `cdn`** — the public URL that serves the rendition directories (`display/`, `thumbnail/`, plus any extra rendition dirs configured under the `renditions` meta key in `/m/instance`) and `gallery-icons/`, typically the same nginx host:
...
/var/photo-diary/
  dailybw/                              # one directory per instance
    ...
    photos/
      inbox/  original/  thumbnail/        # rendition dirs (display/, etc.) auto-created on intake
  travel/
    .env
...
One server block per instance, proxying to its `PORT` (set in the instance's `.env`). nginx also serves the photo rendition directories and `gallery-icons/` directly from disk — the server never streams photos, only their metadata. The `cdn` meta row (set via `./bin/meta.ts set instance_cdn …` — or the [API](server/README.md) / raw SQL as fallback) tells the frontend which URL to load images from; typically the same host the API is on.
...
server {
  ...
  # immutable. One `location` per public rendition directory; add a
  # block for every name listed under the `renditions` meta key (e.g.
  # `display-2500/` if you've configured a larger variant).
  location /display/ {
...
- `<instance>/photos/{original,thumbnail}/` and every configured rendition directory (`display/`, plus anything else under the `renditions` meta key) — the bytes themselves. `inbox/` is transient (the converter empties it), no need to back it up.
