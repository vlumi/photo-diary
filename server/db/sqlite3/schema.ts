...
export interface Photo {
  ...
  isPrivate: boolean;
  // Display ladder. Each entry says "this photo has a rendition at
  // `photos/display/<maxDim>/<id>.jpg`". The SPA builds `<img srcset>`
  // from the list, browser picks the closest fit. Aspect is uniform
  // across the ladder (sharp's downscale preserves it), so the
  // per-rendition dimensions don't need to be stored — `maxDim` (the
  // bounding-square edge) is the only discriminator.
  renditions: number[];
}
