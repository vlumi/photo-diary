...
/opt/photo-diary/                       # parent dir, owned by the deploy user (see below)
  ...
  1.0.4/                               #   so different instances can run different versions
                                        #   and upgrades are atomic (flip a symlink)

/var/photo-diary/
  dailybw/                              # one directory per instance
    .env                                # per-instance config (see below)
    code -> /opt/photo-diary/1.0.4      # symlink to the code version this instance runs
...
```sh
/opt/photo-diary/1.0.4/bin/instance.ts /var/photo-diary/dailybw
```
...
/opt/photo-diary/1.0.4/bin/instance.ts /var/photo-diary/dailybw    # backs up the DB, flips the symlink
...
```sh
/opt/photo-diary/1.0.4/bin/instance.ts gc
```
